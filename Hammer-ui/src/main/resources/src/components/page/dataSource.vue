    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-date"></i> 监控</el-breadcrumb-item>
    ...
    export default {
        ...
        watch:{
            showingSqlId:function(showingSqlId){
                this.$http.get(contextPath + '/druid/sql-'+showingSqlId+'.json').then(resp=>{
                    this.showingSql = resp.body.Content;
                },this.globalFail);
            },
            refresh:function(t){
        ...
        methods: {
            ...
            loadDataSource(){
                //加载数据源信息
                this.$http.get(contextPath + '/druid/datasource.json').then(resp=>{
                    var datasource = this.$data.datasource=[];
                    this.$data.activeDataSource = resp.body.Content[0].Identity + '';
                    for(var i = 0 ;i<resp.body.Content.length;i++){
                        var d = resp.body.Content[i];
                        datasource.push({
                            name:d.Name,
                            id:d.Identity+'',
            ...
            loadSqls(){
              //加载sql监控
                this.$http.get(contextPath + '/druid/sql.json').then(resp=>{
                    this.$data.sql = resp.body.Content;
                },resp=>{
                    this.globalFail(resp);
                });
            },
        ...
        created:function () {
            this.$http.get(contextPath + '/druid/basic.json').then(resp=>{
                this.$data.main = this.map2List(resp.body.Content);
            },resp=>{
                this.globalFail(resp);
            });
        }
